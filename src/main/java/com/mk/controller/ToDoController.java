...
public class ToDoController {
	...
	public ResponseEntity<ApiResponse<?>> getList() {
		return ok(ApiResponse.builder().status(HttpStatus.OK)
						.messages(toDoService.findAllList()).build());
	}

	@PostMapping(value = UrlConstants.LIST + "/{listName}")
	public ResponseEntity<ApiResponse<?>> createList(@Validated @Size(max = 100,
					message = ErrorConstants.LINE_NAME_VALIDATION_MESSAGE) @PathVariable
	String listName,
					Authentication authentication)
					throws ServiceException {
		return ok(ApiResponse.builder().status(HttpStatus.OK)
						.messages(toDoService.createList(authentication, listName))
						.message("TodoLists Created successfully !!").build());
	}
	...
	public ResponseEntity<ApiResponse<?>> updateList(@PathVariable Long listId,
					@Validated @Size(max = 100,
									message = ErrorConstants.LINE_NAME_VALIDATION_MESSAGE)
					@RequestParam
					String listName,
					Authentication authentication) {
		return ok(ApiResponse.builder().status(HttpStatus.OK)
						.messages(toDoService.updateList(authentication, listId, listName))
						.message("TodoLists Name updated successfully !!").build());
	}

	@PreAuthorize("@userSecurity.hasListAccess(authentication,#listId)")
	@DeleteMapping(value = UrlConstants.LIST + "/{listId}")
	public ResponseEntity<ApiResponse<?>> deleteList(@PathVariable Long listId,
					Authentication authentication) {
		return ok(ApiResponse.builder().status(HttpStatus.OK)
						.messages(toDoService.deleteList(authentication, listId))
						.message("TodoLists Deleted successfully !!").build());
	}
	...
	public ResponseEntity<ApiResponse<?>> getAllTask(@PathVariable Long listId,
					Authentication authentication) {
		return ok(ApiResponse.builder().status(HttpStatus.OK)
						.messages(toDoService.getAllTask(listId)).build());
	}
	...
	public ResponseEntity<ApiResponse<?>> addTask(@PathVariable Long listId,
		...
		return ok(ApiResponse.builder().status(HttpStatus.OK)
						...
						.message("Task added successfully !!").build());
	}

	@PreAuthorize("@userSecurity.hasListAccess(authentication,#listId)")
	@PutMapping(value = UrlConstants.LIST + "/{listId}" + "/task" + "/{taskId}")
	public ResponseEntity<ApiResponse<?>> updateTask(@PathVariable Long listId,
