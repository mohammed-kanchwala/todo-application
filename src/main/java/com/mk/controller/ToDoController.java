...
public class ToDoController {
    ...
    public ResponseEntity<ApiResponse<?>> createList(@Max (value =
            100, message = ErrorConstants.LINE_NAME_VALIDATION_MESSAGE) @PathVariable String listName,
                                                     Authentication authentication) throws ServiceException {
        return ok(ApiResponse.builder()
                .status(HttpStatus.OK)
                .messages(toDoService.createList(authentication, listName))
                .message("TodoLists Created successfully !!")
                .build());
    }
    ...
    public ResponseEntity<ApiResponse<?>> updateList(@PathVariable Long listId,
                                                     @Max (value =
                                                             100, message = ErrorConstants.LINE_NAME_VALIDATION_MESSAGE) @RequestParam String listName,
                                                     Authentication authentication) {
        return ok(ApiResponse.builder()
                .status(HttpStatus.OK)
                .messages(toDoService.updateList(authentication, listId, listName))
                .message("TodoLists Name updated successfully !!")
                .build());
    }
    ...
    public ResponseEntity<ApiResponse<?>> deleteList(@PathVariable Long listId,
        ...
        return ok(ApiResponse.builder()
                ...
                .message("TodoLists Deleted successfully !!")
                .build());
    }
    ...
    public ResponseEntity<ApiResponse<?>> updateTask(@PathVariable Long listId,
                                                     ...
                                                     @Valid @RequestBody ToDoDto todoTask) throws ServiceException {
        return ok(ApiResponse.builder()
                .status(HttpStatus.OK)
